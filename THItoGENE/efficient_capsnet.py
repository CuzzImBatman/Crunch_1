...
class RoutingLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        c = torch.einsum("...ij,...kj->...i", u, u)[..., None].cuda()
        # print( self.dim_capsules.is_cuda)
        sqrt= torch.sqrt(torch.Tensor([self.dim_capsules]).to("cuda").type(torch.FloatTensor)).to("cuda")
        c= c/sqrt
        # c = c / torch.sqrt(torch.Tensor([self.dim_capsules]).to("cuda").type(torch.FloatTensor))
