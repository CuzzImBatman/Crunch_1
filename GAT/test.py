...
def load_checkpoint(epoch, model, optimizer,args):
    try:
    ...
    return epoch + 1, args,model,optimizer
import anndata
...
def main(args):
    ...
    model = WiKG(dim_in=args.embed_dim, dim_hidden=1024
                 , topk=6, n_classes=args.n_classes, agg_type='bi-interaction'
                 , dropout=0.3, pool='mean',deivce=device).to(device)
    ...
    if start_epoch>0:
        start_epoch, args, model, optimizer = load_checkpoint(start_epoch, model, optimizer,args)
