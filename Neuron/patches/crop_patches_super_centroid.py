...
for group in group_type:
    for name in NAMES:
        try:
            ...
            with open(f'{cluster_path}/{name}_clusters.pkl','rb') as f:
            ...
            # with open(f'{cluster_path}/{name}_kmeans.pkl','rb') as f:
            #             kmeans= pickle.load(f)
        centroids = cell_locations.groupby('super_cluster')[['x', 'y']].mean().sort_index().reset_index().to_numpy()
    ...
    # Filter out invalid clusters (those with 'train' = -1)
        ...
        valid_clusters= cell_locations['super_cluster'].unique() 
